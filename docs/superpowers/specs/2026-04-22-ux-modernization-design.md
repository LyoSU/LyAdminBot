...
### 9. Уніфіковане повідомлення про авто-дії — **compact-by-default** *(центральне)*

**Проблема:** зараз є зоопарк (`spam.banned`, `spam.muted`, `spam.notification.{full,muted_only,deleted_only,no_permissions}`, `global_ban.kicked`, `spam_vote.title_*`, `report.spam_found`) — і всі шлють великі картки тексту. Засмічує чат, і адмін все одно бачить не те, що йому треба швидко.

**Рішення:** **один рядок за замовчуванням, деталі — за кліком.** Натхненно "Lols Anti Spam" / CAS-ботами. Контракт: довіряй боту, дай можливість сумніватися.

#### Default state — один рядок
...
- `[ 🤨 За що? ]` бачать **усі** в чаті (в т.ч. сам забанений) — будь-хто має право знати
- `[ ✓ Сховати ]` семантично адмін-онлі, але **рендериться всім** — Telegram відправляє одну inline-клавіатуру на повідомлення для всіх глядачів, per-viewer приховування недоступне; callback-хендлер відхиляє клік не-адміна з toast `menu.access.only_admins`
- Auto-delete: 90s якщо ніхто не натиснув

#### Compact-рядок per-status

| Дія бота | Рядок |
|---|---|
| Бан + delete (high confidence) | `⛔ {імʼя} — спам` |
| Mute 24h (mid confidence, voting не активний) | `🔇 {імʼя} — підозріло · муть 24 год` |
| Тільки видалено (no perms) | `🗑 {імʼя} — видалено` |
| Без прав, тільки попередження | `👀 {імʼя} — підозрілий допис` |
| Глоб-бан з іншого чату | `🌍 {імʼя} — глобальний бан` |
| Voting активний (mid confidence) | `⚖️ {імʼя} — на голосуванні` *(різні кнопки, див. нижче)* |
| Адмін override | `↩️ Скасовано адміном @x` *(фінальний стан, auto-delete 30s)* |

**Username як скорочення:** якщо є `@username` → `@username`; якщо лише first_name → `{first_name}`; якщо channel → `{channel_title}`; якщо ID-only → `id{N}` (як в "Lols Anti Spam").

#### Expanded state — натиск `[ 🤨 За що? ]`

Той самий повідомлення редагується (без нового повідомлення в чаті):

```
⛔ {імʼя} — спам

📊 87% · 🤖 схоже на відомий фішинг
📝 «купи курс трейдинга, 100% профіт за тиждень...»
```

Кнопки:

```
[ 🛡 Зменшити деталі ]   [ ↩️ Розблокувати ]
[ ✓ Сховати ]
```

- `[ 🛡 Зменшити деталі ]` — повертає компактний стан
- `[ ↩️ Розблокувати ]` — admin-only, замінює всю картку на `↩️ Скасовано адміном @x` + auto-delete 30s
- `[ ✓ Сховати ]` — admin-only, видаляє повідомлення повністю

#### Visual rules (важливо для всього шару)

- **Без bold заголовків** — один рядок `{emoji} {name} — {reason-tag}`
- **Без user_context** (репутація, вік аккаунту, к-ть повідомлень) у default — це шум; показуємо тільки в expanded
- **Без message preview** у default — теж тільки в expanded
- **Confidence + reason** в expanded — компактно одним рядком: `📊 87% · 🤖 {short reason}`
- Якщо щось пішло не так (видалити не вдалось, мут не пройшов) — додатковий рядок `⚠️ {warning}` тільки в expanded

#### Кнопки уніфіковано (ремап)

| Сценарій | Default | Expanded |
|---|---|---|
| Авто-бан/мут | `[ 🤨 За що? ]` всім + `[ ✓ Сховати ]` адмінам | `[ 🛡 Зменшити ] [ ↩️ Розблокувати ]` + `[ ✓ Сховати ]` адмінам |
| Voting активний | `[ 🚫 N ] [ ✓ N ]` voting-eligible + `[ 🤨 За що? ]` | `[ 🚫 N ] [ ✓ N ]` + `[ 🛡 Зменшити ]` |
| Без прав | `[ 🤨 Що сталось? ] [ 📖 Дай права ]` адмінам | те саме + повна інструкція в тілі |
| Глоб-бан | `[ 🤨 За що? ]` всім + `[ ↩️ Все одно довірити ]` адмінам | + причина з global ban DB |

Усі callbacks → `m:v1:mod.event:<action>:<eventId>` через menu router. `eventId` — з `SpamVote` для voting-кейсів, з нової легкої колекції `ModEvent` для решти.

#### i18n keys

Новий namespace `mod_event:`:
- `compact.{banned, muted, deleted, suspicious, global_ban, voting, override}` — однорядкові шаблони з `{name}` placeholder
- `expanded.{header, confidence_line, preview_line, warning_line}`
- `btn.{why, why_alt, less, hide, undo, vote_spam, vote_clean, give_rights, trust_anyway, cant_delete}`

Старі ключі (`spam.banned`, `spam.muted`, `spam.notification.*`, `global_ban.kicked`) — видаляємо у тій самій версії (compact формат принципово несумісний з попередніми full-card шаблонами; deprecated-alias тут не має сенсу).

#### Auto-cleanup

- Default compact → **90s** якщо ніхто не клікнув
- Expanded → **120s** після останнього кліку
- Override → **30s**
- Voting активний → до резолюції + 60s
